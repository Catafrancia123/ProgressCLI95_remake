...
def beginMenu(systemname, systemlevel, systempro):
    ...
    if choice == "1":
        clear()
        print(lang.gamemodepick)
        rprint("1. "lang.gamemodenormal)
        rprint("2. "lang.gamemodepbsweeper)
        pick = input("> ")
        if pick == "1" or choice == "one":
            startGame(systemname, systemlevel, systempro)
        elif pick == "2" or choice == "two":
            startGamemine(systemName)
        else:
            settings(systemname, systemlevel, systempro)
    elif pick == "2":
        beginMenu(systemname, systemlevel, systempro)
...
def startGamemine(systemName, startLevel, proLevel)
    global mineprogressbar #blue bars
    global mineprogressbar2 #orange bar
    global lives
    global score
    global bar
    global bardisplay
    ...
    while True:
        # clears the screen for next pick
        clear()
