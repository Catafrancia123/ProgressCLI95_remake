class language():
  # contibuted by markverb1
  # bg_BG language configuration file
  # {*} <-- these are for python's .format() command, used for placing variables in strings.
  # Please do not remove \n, {*}, or [] objects.

  # boot
  pleaseSelect = 'Моля изберете вашия език като въведете езиковия код отляво (не е съвместим с 0.24)'
  sparrow = '[white]Спароу асистент подобрен текст BIOS.[not bold]80.1[/not bold][/white] - [bright_yellow]Енергийна Звезда (не)Захранено[/bright_yellow]'
  # {0} is for the build verson, {1} is for the compile date.
  version = '[white]CLI версия [bold]{0}[/bold] - компилиран {1}[/white]'
  dev = '[bold red]- СГРАЖДАНЕ ЗА РАЗРАБОТКА // ГРЕШКИТЕ МОЖЕ ДА СА ЧЕСТИ // ЕЗИКЪТ НЕ Е СЪВМЕСТИМ С ПОСЛЕДНАТА ВЕРСИЯ -[/bold red]\n\n'
  # {0} is for the boot menu list number, {1} is for the full system name, {2} is the level number required for unlock, {3} is the level you need in a specified system to unlock this one.
  notUnlocked = '[red][not bold]{0}[/not bold]. [not bold]{1}[/not bold] - Стигнете до ниво {2} в {3} за да отключите това![/red]'
  credits1 = "Екип от [blink]програмисти[/blink]"
  credits2 = "Екип от [blink]преводачи[/blink]"

  # player
  loading = '\n\n\nСега се зарежда...'
  bar = '\nВашата лента:'
  barProgress1 = "\nВие имате {0}% с {1}% оранжево във вашата лента за прогрес."
  barProgress2 = "\nВие имате {0}% във вашата лента за прогрес"
  sett1 = 'Н а с т р о й к и'
  sett2 = 'Поп-ъп'
  sett3 = 'Назад'
  doYouWant = "Искате ли да имате това:"
  popupSetting = "\nВ изскачащия прозорец? (Y/N)"
  wait = 'М о л я  и з ч а к а й т е\n\n\n'
  closeCMD = '[bold yellow]Вече е безопасно да затворите конзолата ви.[/bold yellow]'
  bm1 = 'Старт меню'
  bm2 = 'Нова игра'
  bm3 = 'Зарежнаде на игра'
  bm4 = 'Рестартиране'
  bm5 = 'Изключване'
  bm6 = 'Настройки'
  bm7 = 'Продължете'
  annoyingPopup = ':) Дразнещ поп-ъп!'
  outOfLives = "[bold bright_blue]Вече нямате животи. Край на играта![/bold bright_blue]"
  noLevelTaken = '[i]Не е взето едно ниво.[/i]'
  negateLevel = '[bold i]-1 ниво[/bold i]'
  level = 'Ниво'
  oneLifeLeft = "Вие имате [italic bright_red]1 живот останал[/italic bright_red]. Бъдете внимателни."
  # {0} is the current number of lives.
  livesLeft = "Вие имате {0} животи останали."
  pressInstructions = "Напишете 'c' за да уловите, нещо друго за да не уловите, и 'q' за да спрете играта.\n"
  gameOver = 'Край! Благодаря ви, че играхте!'
  pressEnter = '\nНатиснете ENTER за да играте друго ниво.'
  gameBravo = 'Браво!'
  gamePerfect = 'Перфектно!'
  gameOuterSpace = 'В космоса!'
  gameYinAndYang = 'Ин и ян'
  gameNonconformist = "Некомфист!"
  newSystem = '[i]Нова система е отключена...[/i]'
  proCongrats = '\nБраво! Вие сте професионалиста!'
  proAcquired = 'Вече имате значката "Про"!'
  expertAcquired = '\nЕкспертна значка получена!'
  masterAcquired = '\nМайсторска значка получена!!'
  adeptAcquired = '\n"Адепт" значка получена!'
  grandAcquired = '\nВеликата значка получена!'
  grandmaster = "\nЕтикетът Велик майстор е придобит!"
  discordmod = "\nДокоснахте ли вече дори трева??"
  whatAcquired = '\nКакво?'
